...
public class PlaywrightAPITest {
  ...
  void shouldGetAllPersons() {
    ...
    Person firstPerson = new Gson().fromJson(json.get(10), Person.class);

    assertEquals(firstPerson.uname(), "John");
    assertEquals(firstPerson.email(), "john.doe@example.com");
  }
  ...
  void shouldGetPerson() {
    ...
    APIResponse response = request.get("/person/15");

    Person person = new Gson().fromJson(response.text(), Person.class);

    assertEquals(200, response.status());
    // assertEquals("OK", response.statusText());
    assertEquals(person.uname(), "John");    
    assertEquals(person.email(), "john.doe@example.com");
  }

  @Disabled
  void shouldDeletePerson() {
    // http://localhost:10000/person/6
    APIResponse response = request.delete("/person/6");
  ...
  void shouldUpdatePerson() {
    ...
    APIResponse response = request.put("/person/15", RequestOptions.create().setData(data));
    Person person = new Gson().fromJson(response.text(), Person.class);
